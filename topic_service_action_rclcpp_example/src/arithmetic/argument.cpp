...
void Argument::publish_random_arithmetic_arguments()
{
  ...
  RCLCPP_INFO(this->get_logger(), "Published argument_a %2.f", msg.argument_a);
  RCLCPP_INFO(this->get_logger(), "Published argument_b %2.f", msg.argument_b);
}
...
void Argument::update_parameter()
{
  ...
  parameter_event_sub_ = parameters_client_->on_parameter_event(parameter_event_callback);
}
