...
void Checker::send_goal_total_sum(float goal_sum)
{
  ...
  if (!this->arithmetic_action_client_->wait_for_action_server(std::chrono::seconds(10))) {
    RCLCPP_WARN(this->get_logger(), "Arithmetic action server is not availble.");
    return;
  }
...
void Checker::get_arithmetic_action_goal(
  std::shared_future<GoalHandlerArithmeticChecker::SharedPtr> future)
{
...
void Checker::get_arithmetic_action_feedback(
  GoalHandlerArithmeticChecker::SharedPtr,
...
void Checker::get_arithmetic_action_result(
  const GoalHandlerArithmeticChecker::WrappedResult & result)
{
  switch (result.code) {
    case rclcpp_action::ResultCode::SUCCEEDED:
      RCLCPP_INFO(this->get_logger(), "Action succeded!");
